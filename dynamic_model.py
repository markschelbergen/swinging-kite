...
def derive_tether_model_kcu_williams_wo_wing(n_tether_elements, vwx=0):
    ...
    r = ca.vec(r.T)[:(n_elements - 1) * 3]
    v = ca.vec(v.T)[:(n_elements - 1) * 3]
    f = ca.vec(f.T)
...
def dae_sim(tf, n_intervals, dyn):
    ...
    z = ca.vertcat(a[:(dyn['n_elements'] - 1) * 3], nu)
