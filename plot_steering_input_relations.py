...
def plot_relations():
    ...
    plt.subplots_adjust(top=0.985, bottom=0.085, left=0.145, right=0.99, hspace=0.1, wspace=0.225)
    ax[0, 0].set_ylabel("Steering input [%]")
    ax[1, 0].set_ylabel("Pitch difference [$^\circ$]")
    ...
    r = np.corrcoef(flight_data.kite_actual_steering, flight_data.pitch0_tau-flight_data.pitch1_tau)[0, 1]
    ax[1, 1].text(.55, .7, "R={:.2f}".format(r), transform=ax[1, 1].transAxes)
    ...
    r = np.corrcoef(flight_data.kite_actual_steering, flight_data.roll0_tau)[0, 1]
    ax[2, 1].text(.55, .7, "R={:.2f}".format(r), transform=ax[2, 1].transAxes)
    ...
    r = np.corrcoef(flight_data.kite_actual_steering, flight_data.kite_1_yaw_rate)[0, 1]
    ax[3, 1].text(.55, .7, "R={:.2f}".format(r), transform=ax[3, 1].transAxes)
    ...
    add_panel_labels(np.delete(ax.reshape(-1), 1), [.24, .24, .38, .24, .38, .24, .38])
